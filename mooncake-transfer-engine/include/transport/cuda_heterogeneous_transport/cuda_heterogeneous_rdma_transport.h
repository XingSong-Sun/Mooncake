...
#ifndef CUDA_HETEROGENEOUS_RDMA_TRANSPORT_H_
#define CUDA_HETEROGENEOUS_RDMA_TRANSPORT_H_
...
#define CUDA_HETERO_HOST_SIZE 3ULL * 1024 * 1024 * 1024
#define CUDA_HETERO_DEVICE_SIZE 8 * 1024 * 1024
#define CUDA_HETERO_DEVICE_NUM 4
#define CUDA_HETERO_AGGREGATE_SIZE_LIMIT 2 * 1024 * 1024

namespace mooncake {

class CudaHeterogeneousRdmaTransport : public Transport {
   public:
    CudaHeterogeneousRdmaTransport()
    ...
    const char *getName() const override { return "cuda_hetero"; }
   ...
   private:
    ...
    char *acquireBlock() {
        ...
        for (size_t i = 1; true; ++i) {
            ...
            if (block_queue_cv_.wait_for(lock, 1000ms, [&] {
                ...
                LOG(INFO)
                    << "CudaHeterogeneousRdmaTransport: acquireBlock time "
                       "out, idx:"
                    << i;
            }
        }
    ...
    bool allBlockReleased() const {
        return block_queue_.size() == (size_t)CUDA_HETERO_DEVICE_NUM;
    }
    ...
    TransferInfo getTransfer() {
        ...
        for (size_t i = 1; true; ++i) {
            ...
            if (transfer_queue_cv_.wait_for(lock, 10000ms, [&] {
                    return !this->transfer_queue_.empty() || !this->running_;
                })) {
...
#endif  // CUDA_HETEROGENEOUS_RDMA_TRANSPORT_H_
